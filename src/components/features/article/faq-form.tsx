...
export default function FaqForm({
  ...
  const wordLengthOptions = [
    { value: "60", label: t("short") },
    { value: "120", label: t("middle") },
    { value: "240", label: t("long") },
  ];
  ...
  const handleFormSubmit = (data: FaqFormValues) => {
    ...
    if (!data.wordLength) {
      setError("wordLength", { message: t("form.errorRequired") });
      hasError = true;
    }
    if (!data.language) {
    ...
    const payload: Record<string, unknown> = {
      ...
      word_count: parseInt(data.wordLength),
      question_count: parseInt(data.askCount),
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 mt-5">
      <Input
        label={t("form.topic")}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Input
          label={t("form.askCount")}
          placeholder={t("form.askCountPlaceholder")}
          type="number"
          error={errors.askCount?.message}
          {...register("askCount", {
            required: t("form.errorRequired"),
            min: {
              value: 5,
              message: t("form.minQuestion", { count: "5" }),
            },
            max: {
              value: 20,
              message: t("form.maxQuestion", { count: "20" }),
            },
          })}
        />
      </div>
      ...
      <Select
        label={t("form.wordLength")}
        placeholder={t("form.wordLengthPlaceholder")}
        options={wordLengthOptions}
        value={watch("wordLength")}
        onChange={(val) => {
          setValue("wordLength", val);
          clearErrors("wordLength");
        }}
        error={errors.wordLength?.message}
      />
