...
export default function Humanizer({ id }: HumanizerProps) {
  const t = useTranslations("humanizer");
  const tRoot = useTranslations();
  const router = useRouter();
  const { goBack, canGoBack } = useBackNavigation(/^\/ai-tools\/humanize(\/.*)?$/);
  ...
  const writingStyleOptions = useMemo(
    () => [
      ...
      { label: t("styles.informal"), value: "informal" },
  ...
  const handleHumanize = useCallback(async () => {
    ...
    if (!writingStyle) {
      newErrors.writingStyle = tRoot("article.form.errorRequired");
    }
    if (!language) {
        ...
        },
        {
          ...
          onCompleted: (result: any) => {
            ...
            if (typeof result?.article_id === "string") {
              ...
              window.history.replaceState(null, "", `/${locale}/ai-tools/humanize/${result.article_id}`);
              // Fetch title if not in result
              if (!result.title) {
          ...
          onFailed: (message) => {
            hasStreamError = true;
            toast.error(message || t("humanizeError"));
          },
        },
      );
    } catch (err) {
  ...
  }, [inputText, writingStyle, language, t, tRoot, flushStreamToUI]);
      ...
      <div className="flex flex-col h-[calc(100vh-var(--header-height))] overflow-hidden">
            ...
            {(id || lastArticleId) && (
              <Button
                ...
                onClick={() => {
                  ...
                  setProgressMessage("");
                  streamedContentRef.current = "";
                  router.push(`/${Cookies.get("NEXT_LOCALE") || "en"}/ai-tools/humanize`);
                }}
              >
                {t("newHumanize")}
              </Button>
        ...
        <div className="flex-1 flex items-stretch justify-center min-h-0 overflow-hidden p-5">
          <div className="w-full max-w-6xl border border-border/70 rounded-xl bg-surface/50 shadow-lg ring-1 ring-border/30 overflow-hidden flex flex-col min-h-0">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-border bg-background/60 backdrop-blur-sm">
              <div className="w-48">
                <Select
                  ...
                  onChange={(val) => {
                    setWritingStyle(val);
                    setFieldErrors((prev) => ({ ...prev, writingStyle: undefined }));
                  }}
              ...
              {isProcessing && (
                <div className="ml-auto hidden sm:flex items-center gap-1.5 rounded-full border border-border bg-surface/70 px-3 py-1 text-xs text-muted max-w-[260px]">
            ...
            <div className="flex flex-col lg:flex-row flex-1 min-h-0">
              <div className="flex-1 flex flex-col border-b lg:border-b-0 lg:border-r border-border min-h-0 overflow-hidden">
                ...
                <div className={cn(
                  "flex-1 min-h-0 p-4 overflow-hidden",
                  uploadedFileName && "pb-14"
                )}>
                  {uploadedFileName && (
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:h-12 sm:items-center sm:justify-between px-4 py-3 sm:py-0 border-t border-border bg-surface/30 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    <Button
                      ...
                      className="rounded-sm"
                    >
                      {isProcessing ? t("processing") : t("humanize")}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                {outputText && (
                  <div className="flex items-center justify-end px-4 py-3 border-t border-border bg-surface/30 flex-shrink-0">
                    <span className="inline-flex items-center gap-1 rounded-full border border-border bg-background/70 px-2 py-1 text-[11px] text-muted-foreground">
                      <span className="uppercase tracking-wide text-[10px]">
                        {t("wordCount")}
                      </span>
